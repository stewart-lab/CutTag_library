...
def main():
    ...
    for sample in sampleList:
        if "IgG" in sample:
            controlList.append(sample)
        else:
            othersampleList.append(sample)
    treatmentList=["Ctrl","Vip","Zen","Combo"]
    repList= ["Rep1","Rep2","Rep3"]
    for treat in treatmentList:
        newsampleList=[]
        newcontrolList=[]
        for sample in othersampleList:
            if sample.startswith(treat):
                newsampleList.append(sample)
        for control in controlList:
            if control.startswith(treat):
                newcontrolList.append(control)
        for rep in repList:
            for sample in newsampleList:
                if sample.endswith(rep):
                    newsamp= sample
            for control in newcontrolList:
                if control.endswith(rep):
                    newctrl= control
            seacr_cmd = (seacr + " " + bowtie2_dir + "/alignment/bedgraph/" + newsamp + "_bowtie2.fragments.normalized.bedgraph " + 
                         bowtie2_dir + "/alignment/bedgraph/" + newctrl + "_bowtie2.fragments.normalized.bedgraph norm stringent "+ 
                         bowtie2_dir + "/peakCalling/SEACR/" + newsamp + "_seacr_control.peaks")
            print(seacr_cmd)
            addl_logfile.write("\n\nR seacr_cmd: " + seacr_cmd + "\n")
            result = run(seacr_cmd, check=True, capture_output=True, text=True, shell=True)
            if (result.returncode):
                print("RC:", result.returncode, "\nOUT:", result.stdout, "\nERR:", result.stderr)
            addl_logfile.write("RC:" + str(result.returncode) + "\nOUT:" + result.stdout + "\nERR:" + result.stderr)
